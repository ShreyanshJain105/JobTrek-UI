    ...
    return <div className="w-2/3">
        <div className="relative">
            ...
            <img className="rounded-full w-48 h-48 -bottom-1/3 absolute left-3 border-mine-shaft-950 border-8"
                src={profile.picture ?
                    `data:image/jpeg;base64,${profile.picture}` : "/Avatar.png"}
                alt="" />
        </div>
        ...
        <div className="px-3">
            ...
            <div className="flex flex-wrap gap-2">
                {
                    profile?.skills?.map((skill: any, index: any) => <div key={index} className="bg-bright-sun-300 text-sm font-medium bg-opacity-15 rounded-3xl text-bright-sun-400 px-4 py-1">{skill}</div>
                    )
                }
        ...
        <div className="px-3 ">
            ...
            <div className="flex flex-col gap-8">
                {
                    profile?.experiences?.map((exp: any, index: any) => <ExperinceCard key={index} {...exp} />
                    )
                }
            </div>
        ...
        <div className="px-3">
            ...
            <div className="flex flex-col gap-8">
                {
                    profile?.certifications?.map((certi: any, index: any) => <CertifyCard key={index} {...certi} />
                    )
                }
            </div>
        </div>
