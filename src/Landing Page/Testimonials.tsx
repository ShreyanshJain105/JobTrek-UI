...
const Testimonials=()=>{
    return <div className="mt-20 pb-5 p-5 ">
        ...
        <div className="flex justify-evenly md-mx:flex-wrap mt-10 gap-5">
            {
                testimonials.map((data,index)=> <div key={index}  className="flex xs-mx:w-full flex-col gap-3 w-[23%] md-mx:w-[48%]  border-bright-sun-400 p-3 border rounded-xl">
                <div className="flex gap-2 items-center">
                    <Avatar className="h-14 w-14" src="avatar.png" alt="itsme"/>
                    <div>
                        <div className="text-lg text-mine-shaft-100 text-semibold sm-mx:text-base xs-mx:text-sm">{data.name}</div>
                        <Rating value={data.rating} fractions={2} readOnly />
                    </div>
                </div>
                <div className="tetx-xs text-mine-shaft-300">{data.testimonial}</div>
            </div>)
            }
        </div>
